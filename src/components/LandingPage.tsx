...
export default function LandingPage({ onSubmit }: LandingPageProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('Form submitted, URL:', trimmedUrl);
    if (trimmedUrl) {
      // Accept any input - let the backend handle validation
      console.log('Calling onSubmit with URL');
      onSubmit(trimmedUrl);
    } else {
    ...
    <div className="pt-16">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="max-w-2xl mx-auto mb-8">
              <div className={`relative flex items-center rounded-2xl transition-all duration-500 ${
                ...
                <button
                  type="submit"
                  disabled={!url.trim()}
                  className="btn-primary absolute right-3 px-5 sm:px-7 py-3 sm:py-3.5 bg-gradient-to-r from-primary to-accent rounded-xl text-white font-semibold flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:transform-none active:scale-95 transition-transform"
                >
            ...
            <div className="flex flex-wrap items-center justify-center gap-4 mb-4">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg glass-card text-sm text-text-secondary hover:text-white transition-all hover:border-primary/30 cursor-pointer"
              >
      ...
      <section className="py-28 relative">
        <div className="absolute inset-0">
          ...
          <div className="absolute top-1/3 right-0 w-[400px] h-[400px] bg-accent/10 rounded-full blur-[150px]" />
        </div>
